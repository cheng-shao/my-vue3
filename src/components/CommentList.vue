<template>
  <div class="comment-list">
    <div v-for="(item, index) in list" :key="index" class="comment-list-item">
      <div class="parent-reply reply">
        ...
        <div v-if="showControl" class="control">
          <el-button class="reply-btn" type="text" @click="handleToReply(index)"
            >回复</el-button
          >
          <el-button
            class="delete-btn"
        ...
        >
          <div class="child-reply reply">
            ...
            <div class="child-content">
              <span> 回复 @{{ child.replyUser.realname }}： </span>
...
export default {
  name: 'CommentList',
  emits: ['close-other-reply', 'delete'],
  components: {
    ReplyComment
  },
  props: {
  ...
  methods: {
    ...
    handleReply(data) {
      ...
      if (f_comment) {
        ...
        params[this.config.replyKey] = this.getReplyValue(
          f_comment,
          this.config.replyValueKey
        )
      }
      if (c_comment) {
        params[this.config.replyKey] = this.getReplyValue(
      ...
      this.config
        .addRequest({ ...params, ...(this.config.addParams || {}) })
        .then((res) => {
          res.data.user = {
