    ...
    >
      <div class="dialog-body">
        ...
        <div v-if="stepsActive == 1 && !exportLoading" class="step-section">
          <flexbox direction="column" align="stretch" class="field">
        ...
        <div v-if="stepsActive == 2" class="step-section">
          <div class="success">
      ...
      <template #footer>
        <div v-if="!exportLoading" class="dialog-footer">
          <el-button v-if="stepsActive == 1" type="primary" @click="sureClick"
            >立即导出</el-button
          >
          <el-button v-else type="primary" @click="closeView">确认</el-button>
...
export default {
  ...
  methods: {
    ...
    getFieldList() {
      ...
      const request =
        this.crmProps.fieldRequest ||
        (this.crmProps.isSeas
          ? filedGetPoolTableFieldAPI
          : filedGetTableFieldAPI)
      request(params)
        .then((res) => {
          res.data = res.data.filter((o) => {
            ...
            return (
              o.formType &&
              !['handwriting_sign', 'desc_text'].includes(o.formType)
            )
          })
          const fieldList = []
          const selectionList = []
