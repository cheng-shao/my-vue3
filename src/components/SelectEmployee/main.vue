<template>
  <div class="employee-department">
    ...
    <div v-if="!radio || showPreview" class="checked-content">
      <div class="checked-top">
        <span class="select-info"
          >已选择<span class="select-info--num">{{
            userDataValue.length + depDataValue.length
          }}</span
          >项</span
        >
...
export default {
  ...
  computed: {
    ...
    depObjArray() {
      return this.depList.filter((item) =>
        this.depDataValue.includes(item.deptId)
      )
    },
    userObjArray() {
      return this.userList.filter((item) =>
        this.userDataValue.includes(item.userId)
      )
    }
  },
  watch: {
  ...
  methods: {
    ...
    selectDelete(deleteItem, type) {
      if (type === 'user') {
        const removeIndex = this.userDataValue.findIndex(
          (o) => o == deleteItem.userId
        )
        if (removeIndex !== -1) {
          this.userDataValue.splice(removeIndex, 1)
        }
      } else if (type === 'dep') {
        const removeIndex = this.depDataValue.findIndex(
          (o) => o == deleteItem.deptId
        )
        if (removeIndex !== -1) {
     ...
     */
    valueChange() {
      this.$emit('select', this.userObjArray, this.depObjArray)
    },
