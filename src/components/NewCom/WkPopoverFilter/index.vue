    ...
    <div style="padding: 3px 12px">
      ...
      >
        <template #default="{ data }">
          <div v-if="data && data.formType == 'range'" class="wk-range-select">
            <flexbox class="wk-range-select__top">
              <el-input-number
                v-model="from[data.field][0]"
                :precision="
                  data.hasOwnProperty('precision') ? data.precision : undefined
                "
                maxlength="100"
                :controls="false"
              />
...
export default {
  ...
  emits: ['show', 'hide', 'update:fieldFrom', 'sure', 'reset'],
  components: {
    WkForm
  },
  inheritAttrs: false,
