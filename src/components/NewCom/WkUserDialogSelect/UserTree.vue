  ...
  <div class="user-tree">
    ...
    <div class="user-tree__bd">
      <template v-if="currentBreadcrumb">
        <!-- 顶级没有全部 -->
        <div
          v-if="breadcrumbs.length > 1 && !radio && !disabled"
          class="all-check"
        >
          <label
            class="el-checkbox"
            :class="[{ 'is-checked': isAll }]"
            @click.stop="allCheck"
            ><span
        ...
        >
          ...
          <div
            class="dept-cell__hd text-one-line"
            :title="item[deptConfig.label]"
          >
            {{ item[deptConfig.label] }}
          </div>
          <i
            v-if="
...
export default {
  ...
  methods: {
    ...
    breadcrumbsClick(item, index) {
      ...
      if (this.isEdit) {
        this.isEdit = false
        this.findAllParentInfo(
          this.tree,
          this.currentBreadcrumb[this.deptConfig.value]
        )
      }
    ...
    deptClick(item) {
      if (
        ...
        if (
          this.breadcrumbs[this.deptConfig.value] !==
          item[this.deptConfig.value]
        ) {
    ...
    setAllStatus() {
      this.$nextTick(() => {
        ...
        let isChecked = children
          ? children.every((item) => item.isChecked)
          : true
        if (isChecked) {
