    ...
    <div :key="visible ? 'visible' : 'hidden'" class="main">
      <div class="main-left">
        <div class="main-top">
          ...
          >
            <template #suffix>
              <el-button type="icon" icon="wk wk-sousuo" @click="goSearch" />
            </template>
          </el-input>
        </div>
        ...
        /> -->
          ...
          <div v-if="searchInput" class="search-view">
            <div class="search-view__hd">
              搜索关键字为<span>{{ searchInput }}</span
              >的结果
            </div>
            <div class="search-view__bd">
              <el-checkbox-group
                ...
                :disabled="disabled"
              >
                <div class="letter-block" :data-letter="resultList[0].letter">
                  <div class="header">
                    ...
                    >
                      ...
                      <div class="content">
                        <div
                          ...
                          :title="user[userConfig.label]"
                        >
                          {{ user[userConfig.label] }}
                        </div>
                        <div
              ...
              <el-checkbox-group
                ...
                :disabled="disabled"
              >
                <div class="letter-block" :data-letter="resultList[1].letter">
                  <div class="header">
              ...
              <el-checkbox-group
                ...
                :disabled="disabled"
              >
                <div class="letter-block" :data-letter="resultList[2].letter">
                  <div class="header">
          ...
          <div v-show="!searchInput" class="main-body">
            <div v-if="tabs.length > 1" class="main-tabs">
              <el-button
                ...
                :key="index"
                :style="{
                  width: `calc(${parseInt(100 / tabs.length)}% - 8px)`
                }"
            ...
            >
              <div class="item-cell__hd">{{ userShowTypeName }}</div>
      ...
      <div class="main-right">
        <div class="main-top">
          已选：<template v-if="config.showUser || config.showDisableUser"
            >{{ currentUserValue.length }}个员工 </template
          ><template v-if="config.showDept"
            >{{ currentDepValue.length }}个部门</template
          >
          <el-button
            class="clear-btn"
            type="link"
        ...
        <div class="main-body is-right">
          <div class="main-select">
            <el-tag
              ...
              closable
              disable-transitions
              @close="selectDelete(idKey, index)"
              ><span v-if="!idKey.includes('user')" class="is-dept" />{{
...
export default {
  ...
  watch: {
    ...
    'config.showDisableUser': {
      handler() {
        this.disableUserList = this.config.showDisableUser
          ? this.config.disableUserList || this.userDeptObj.disableUserList
          : []
      }
    }
  },
  ...
  methods: {
    ...
    initAllData() {
      ...
      this.deptList =
        this.config.showUser || this.config.showDept ? deptList : []
      // disableUserList 有值 以 disableUserList  为准，为展示单列
      this.disableUserList = this.config.showDisableUser
        ? this.config.disableUserList || disableUserList
        : []
    ...
    updateSelectData() {
      // 初始化右侧选择数据
      const userSelectData = this.currentUserValue.map(
        (userId) => `user-${userId}`
      )
      const deptSelectData = this.currentDepValue.map(
        (deptId) => `dept-${deptId}`
      )
    ...
    selectDelete(idKey, index) {
      if (idKey.includes('user-')) {
        ...
        const removeIndex = this.currentUserValue.findIndex(
          (userId) => userId == id
        )
        if (index > -1) {
        ...
        this.selectData.splice(index, 1)
        const id = idKey.split('-')[1]
        const removeIndex = this.currentDepValue.findIndex(
          (deptId) => deptId == id
        )
        if (index > -1) {
    ...
    goSearch() {
      // 不展示的信息 不进行搜索值返回
      this.resultList[0].children =
        this.config.showUser && this.searchInput
          ? this.searchUserDept.userList.filter((item) =>
              PinyinMatch.match(item[this.userConfig.label], this.searchInput)
            )
          : []
      this.resultList[1].children =
        this.config.showDept && this.searchInput
          ? this.searchUserDept.deptList.filter((item) =>
              PinyinMatch.match(item[this.deptConfig.label], this.searchInput)
            )
          : []
      this.resultList[2].children =
        this.config.showDisableUser && this.searchInput
          ? (
              this.config.disableUserList || this.searchUserDept.disableUserList
            ).filter((item) =>
              PinyinMatch.match(item[this.userConfig.label], this.searchInput)
            )
          : []
    ...
    getSelectShowName(idKey, userDeptMap) {
      ...
      if (idKey.includes('user')) {
        return data ? data[this.userConfig.label] : ''
      }
      return data ? data[this.deptConfig.label] : ''
    },
