<template>
  <div class="search-view">
    <div class="search-view__hd">
      搜索关键字为<span>{{ search }}</span
      >的结果
    </div>
    <div class="search-view__bd">
      <el-checkbox-group v-model="userChecksValues" @change="userCheckbox">
        <div
        ...
        >
          ...
          <div class="children">
            ...
            >
              ...
              />
              <div class="content">
                <div class="realname text-one-line" :title="user.realname">
                  {{ user.realname }}
                </div>
                <div class="dept-name text-one-line" :title="user.deptName">
                  {{ user.deptName || '无' }}
                </div>
              </div>
            </check-cell>
      ...
      <el-checkbox-group v-model="deptChecksValues" @change="deptCheckbox"
        >>
        <div
...
export default {
  ...
  methods: {
     ...
     */
    goSearch() {
      this.validLetters[0].children = this.search
        ? this.users.filter((item) =>
            PinyinMatch.match(item.realname, this.search)
          )
        : []
      this.validLetters[1].children = this.search
        ? this.stopUsers.filter((item) =>
            PinyinMatch.match(item.realname, this.search)
          )
        : []
      this.validLetters[2].children = this.search
        ? this.depts.filter((item) => PinyinMatch.match(item.name, this.search))
        : []
    },
