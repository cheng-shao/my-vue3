<template>
  <div class="wk-filter-header">
    ...
    >
      ...
      >
        <template #suffix>
          <el-button type="icon" icon="wk wk-sousuo" @click="searchInput" />
        </template>
      </el-input>
      ...
      <div ref="filterTabs" class="tabs">
        ...
        <el-dropdown
          v-if="
            config.tabSetShow || otherTabs.length > 0 || $slots.dropdownMore
          "
          trigger="click"
          @command="otherClick"
        >
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <div :class="{ 'none-other': otherTabs.length === 0 }">
                <slot name="dropdownMore" />
              </div>
              <el-dropdown-item
...
export default {
  ...
  computed: {
    ...
    otherActiveTabName() {
      ...
      const findRes = this.otherTabs.find(
        (o) => o.value === this.currentActiveTab
      )
    ...
    changeTabsSize() {
      this.$nextTick(() => {
        ...
        if (filterTabs) {
          ...
          const children = Array.from(filterTabs.childNodes || []).filter(
            (item) => ['DIV', 'BUTTON', 'SPAN'].includes(item.tagName)
          )
