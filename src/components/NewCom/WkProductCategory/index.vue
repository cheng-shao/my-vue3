...
import { jxcProductQueryProductTypeAPI } from '@/api/jxc/product'
  ...
  methods: {
    setConfig() {
      if (this.type === 'jxc') {
        ...
        this.request = jxcProductQueryProductTypeAPI
    ...
    valueChange(val) {
      ...
      this.$emit('change', {
        ...
        item: this.item,
        valueContent:
          val.length > 0 ? this.$refs.elCascader.getCheckedNodes()[0].label : ''
      })
    }
  }
}
