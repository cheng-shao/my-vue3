          ...
          >
            ...
            <template v-if="item.isDep">
              <div class="dep-name text-one-line">
                {{
                  `${item[config.depLabel]}${
                    item.currentNum > 0 ? `(${item.currentNum})` : ''
                  }`
                }}
              </div>
              <el-button
            ...
            <template v-else>
              ...
              />
              <div class="user-info">
                <div class="user-name text-one-line">
                  {{ item[config.label] }}
                </div>
...
export default {
  ...
  methods: {
    ...
    searchListUser(queryString, cb) {
      if (
        (this.searchUserList && this.searchUserList.length) ||
        this.config.userOptions
      ) {
        const searchUserList = this.config.userOptions || this.searchUserList
        if (queryString) {
          cb(
            searchUserList.filter((item) =>
              PinyinMatch.match(item[this.config.label] || '', queryString)
            )
          )
        } else {
          cb(searchUserList)
        ...
        request(params)
          .then((res) => {
            this.searchUserList = res.data.hasOwnProperty('list')
              ? res.data.list || []
              : res.data || []
            if (queryString) {
              cb(
    ...
    getDepUserList(deptId, depInfo) {
      ...
      request(deptId)
        .then((res) => {
          ...
          if (deptId == 0) {
            this.breadcrumbList = [
              { label: '组织', deptList: deptList, employeeList: employeeList }
            ]
          } else {
    ...
    dataCheckboxChange(isCheck, data) {
      if (data.isDep) {
        if (isCheck) {
          ...
          const index = this.getArrayIndexWithItem(
            this.selectDeps,
            data,
            this.config.depValue
          )
          if (index >= 0) {
