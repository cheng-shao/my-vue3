<template>
  <div class="xh-user">
    <div v-if="headerShow" class="xh-user__hd">部门</div>
...
import { hrmDeptQueryTreeListAPI } from '@/api/hrm/dept'
...
export default {
  ...
  methods: {
    ...
    getDepUserList(deptId, data) {
      if (deptId == 0) {
        this.breadcrumbList = [
          { label: '全部', deptList: data, employeeList: [] }
        ]
      } else {
        this.breadcrumbList.push({
          label: data.name,
          deptList: data.children,
          employeeList: []
        })
      }
    },
    ...
    requestDepList() {
      ...
      const request =
        this.config.dataType === 'hrm'
          ...
          : this.config.request || depListAPI
      request(
        this.config.params || {
          type: 'tree'
        }
