...
export default {
  ...
  methods: {
    ...
    validValue() {
      ...
      if (
        this.value !== this.dataValue.select &&
        this.value !== this.dataValue.otherValue
      ) {
        if (isEmpty(this.value)) {
          ...
          if (this.otherShowInput) {
            const valueObj = this.options.find(
    ...
    valueChange() {
      const value =
        this.dataValue.select === LOGIC_OTHER
          ? this.dataValue.otherValue.trim()
          : this.dataValue.select
    ...
    inputBlur() {
      ...
      const eIsObject =
        this.options.length > 0 && !this.isEmptyValue(this.options[0].value)
