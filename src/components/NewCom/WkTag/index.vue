    ...
    <template #reference>
      ...
      >
        <div
          ...
          class="tag-item text-one-line"
        >
          {{ item.name }}
          <i class="delete-icon el-icon-close" @click.stop="deleteTag(index)" />
        </div>
        <i v-if="dataValue.length > max" class="el-icon-more" />
        <i :class="['el-icon-arrow-up']" />
        <div v-if="dataValue.length == 0" class="add-item text-one-line">
          {{ placeholder }}
        </div>
      </flexbox>
