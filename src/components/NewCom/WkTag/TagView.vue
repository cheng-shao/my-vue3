<template>
  <div class="tag-view" :style="{ 'flex-wrap': wrap }">
    <template v-if="value && value.length <= maxLength">
      <div
        v-for="(item, index) in value"
        :key="index"
        :style="{
          background: colorRgb(item.value, 0.1),
          'margin-bottom': `${itemBottom}px`,
          color: item.value
        }"
        class="item-label"
      >
    ...
    <template v-if="value && value.length > maxLength">
      ...
      >
        ...
        <el-button size="mini" style="height: 20px" icon="el-icon-more" />
      </el-tooltip>
    </template>
