 ...
 * @returns 字符串
 */
export function getFormFieldShowName(
  formType,
  value,
  placeholder = '--',
  item
  ...
  if (formType === 'position') {
    if (isArray(value)) {
      ...
      try {
        ...
        return isArray(valueObj)
          ? valueObj.map((item) => item.name).join()
          : placeholder
      } catch (error) {
        return placeholder
      }
    ...
    const newValue = CustomFieldsMixin.methods.getRealParams(
      { formType },
      value
    )
    if (isEmpty(newValue)) {
    ...
    if (isArray(value)) {
      return (
        value.map((item) => item.name || item.deptName).join() || placeholder
      )
    }
    return value || placeholder
    ...
    if (isArray(value)) {
      return (
        value.map((item) => item.realname || item.employeeName).join() ||
        placeholder
      )
    } else if (isObject(value)) {
      return value.realname || value.employeeName || placeholder
    }
    return value || placeholder
