 ...
 * @LastEditTime: 2022-04-25 15:32:37
 * @LastEditors: yang
-->
<template>
  <el-form ref="form" v-bind="$attrs" :class="customClass" class="wk-form">
    <el-form-item
    ...
    >
      <template #label>
        ...
        >
          <template #content>
            <div v-html="getTips(item)" />
          </template>
          <i class="wk wk-help wk-help-tips" />
        </el-tooltip>
        <i
          v-else-if="item.tipType == 'system-help' || item.helpType"
      ...
      <wk-select
        v-else-if="
          item.formType === 'select' && item.setting.includes(LOGIC_OTHER)
        "
        v-model="fieldFrom[item.field]"
        :disabled="item.disabled"
      ...
      <wk-checkbox
        v-else-if="
          item.formType === 'checkbox' && item.setting.includes(LOGIC_OTHER)
        "
        v-model="fieldFrom[item.field]"
        :disabled="item.disabled"
        :clearable="item.clearable"
      ...
      <wk-dep-select
        v-else-if="
          ...
          (item.request || item.props || item.params)
        "
        v-model="fieldFrom[item.field]"
      ...
      <wk-user-select
        v-else-if="
          ['single_user', 'user'].includes(item.formType) &&
          (item.request || item.props || item.params)
        "
        v-model="fieldFrom[item.field]"
        :request="item.request"
