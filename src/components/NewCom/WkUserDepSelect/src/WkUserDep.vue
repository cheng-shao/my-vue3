<template>
  <div class="xh-user">
    ...
    <flexbox v-if="config.showFooter" class="xh-user__ft">
      <flexbox-item class="select-info"
        >已选择<span class="select-info--num">{{
          (depDataValue.length || 0) + (userDataValue.length || 0)
        }}</span
        >项</flexbox-item
      >
      <el-button type="text" @click="clearAll">清空</el-button>
      <el-button v-if="config.showSureBtn" type="text" @click="sureClick"
        >确定</el-button
      >
    </flexbox>
  </div>
...
import { adminUserQueryByDeptAPI } from '@/api/admin/user'
import {
...
export default {
  ...
  methods: {
      ...
      if ((this.searchUserList && this.searchUserList.length) || this.options) {
        ...
        if (queryString) {
          cb(
            searchUserList.filter(
              (item) =>
                PinyinMatch.match(
                  item[this.userConfig.label] || '',
                  queryString
                ) || PinyinMatch.match(item.deptName || '', queryString)
            )
          )
        } else {
          cb(searchUserList)
        ...
        request(params)
          .then((res) => {
            ...
            this.searchUserList = resData.hasOwnProperty('list')
              ? resData.list || []
              : resData || []
            if (queryString) {
              cb(
                this.searchUserList.filter((item) =>
                  PinyinMatch.match(
                    item[this.userConfig.label] || '',
                    queryString
                  )
    ...
    userAllSelect(allChecked) {
      ...
      if (
        (this.depAllChecked ||
          !this.currentBreadcrumbItem.deptList ||
          this.currentBreadcrumbItem.deptList.length === 0 ||
          this.config.selectType === 'user') &&
        this.userAllChecked
      ) {
    ...
    depAllSelect(allChecked) {
      ...
      if (
        this.depAllChecked &&
        (this.userAllChecked ||
          !this.currentBreadcrumbItem.employeeList ||
          this.currentBreadcrumbItem.employeeList.length === 0 ||
          this.config.selectType === 'dep')
      ) {
        this.allChecked = true
      } else {
        this.allChecked = false
    ...
    getDepUserList(deptId, depInfo) {
      this.loading = true
      const request = this.props.isKpi
        ? queryInspectionDeptEmployeeList
        : this.config.dataType === 'hrm'
        ? hrmEmployeeQueryByDeptAPI
        : this.config.request || adminUserQueryByDeptAPI
      request(deptId)
        .then((res) => {
          const data = res.data || {}
          ...
          if (deptId == 0) {
            this.breadcrumbList = [
              { label: '全部', deptList: deptList, employeeList: employeeList }
            ]
          } else {
