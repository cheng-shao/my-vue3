  ...
  >
    <div class="el-select__tags">
      ...
      <span
        ...
        :key="`dep${index}`"
        class="user-item text-one-line"
        >{{ item[depPropsValue.label] }}
        <i class="delete-icon el-icon-close" @click.stop="deleteDep(index)" />
      </span>
    </div>
    <el-tooltip
      v-if="depSelects.length + userSelects.length > max"
    ...
    <div
      ...
      class="user-placeholder text-one-line"
    >
      {{ placeholder }}
    </div>
...
export default {
  ...
  computed: {
    ...mapState(useUserStore, ['userList', 'deptList']),
    ...
    showDepSelects() {
      let max = 0
      if (
        (this.userSelects && this.userSelects.length < this.max) ||
        !this.userSelects
      ) {
    ...
    userSelects() {
      ...
      return this.userOptionsList.filter((item) =>
        this.userDataValue.includes(item[this.userPropsValue.value])
      )
    },
    ...
    depDataValue(newValue, oldValue) {
      if (!valueEquals(newValue, this.depValue)) {
        if (this.radio) {
          this.$emit(
            'update:depValue',
            this.depDataValue && this.depDataValue.length
              ? this.depDataValue[0]
              : ''
          )
        } else {
    ...
    userDataValue(newVal, oldVal) {
      if (!valueEquals(newVal, this.userValue)) {
        if (this.radio) {
          this.$emit(
            'update:userValue',
            this.userDataValue && this.userDataValue.length
              ? this.userDataValue[0]
              : ''
          )
        } else {
  ...
  methods: {
    ...mapActions(useUserStore, ['debounceGetUserList']),
        ...
        if (!valueEquals(this.depValue, this.depDataValue)) {
          if (this.depValue && this.depValue.length) {
            ...
            if (firstItem[this.depPropsValue.value]) {
              this.depDataValue = this.depValue.map(
                (item) => item[this.depPropsValue.value]
              )
            } else {
        ...
        if (!valueEquals(this.userValue, this.userDataValue)) {
          if (this.userValue && this.userValue.length) {
            ...
            if (firstItem[this.userPropsValue.value]) {
              this.userDataValue = this.userValue.map(
                (item) => item[this.userPropsValue.value]
              )
            } else {
    ...
    getTooltipText() {
      ...
      let strName = structures
        .map((item) => item[this.depPropsValue.label])
        .join('、')

      const users = this.userSelects || []
      const userName = users
        .map((item) => item[this.userPropsValue.label])
        .join('、')
