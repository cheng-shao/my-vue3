...
<template>
  <div class="wk-file-form">
    <wk-file-cell
      ...
      @delete="fileDelete"
    />
    <wk-file-select multiple @change="fileSelect">
...
<script>
import { adminFileUploadAPI, adminFileDeleteByIdAPI } from '@/api/admin/file'
...
export default {
  // 自定义字段附件
  name: 'WkFileForm',
  emits: ['input', 'update:defaultValue', 'change'],
  components: {
    WkFileSelect,
    WkFileCell
  },
  ...
  methods: {
    ...
    fileDelete(data, index) {
      this.loading = true
      adminFileDeleteByIdAPI(data.fileId)
        .then((res) => {
