  ...
  >
    <div class="type-popper">
      <flexbox class="type-popper-common">
        <flexbox direction="column" justify="space-around" class="type-sort">
          <div
            v-for="(item, index) in sortList"
            :key="index"
            class="type-sort-item"
          >
            {{ item.name }}
          </div>
        </flexbox>

        <flexbox direction="column" justify="space-around" class="type-content">
          <flexbox
            v-for="(item, index) in sortList"
            :key="index"
          ...
          >
            <template v-for="(subItem, subIndex) in options">
              <div
                ...
                :class="{
                  selected:
                    selectItem.value == subItem.value && !showCustomContent
                }"
      ...
      <div>
        <div class="type-content-item custom">自定义</div>
        <flexbox justify="space-between" class="type-content-custom">
          <el-date-picker
    ...
    <template #reference>
      <el-input
        ...
        :class="[
          mode ? 'type-select--' + mode : '',
          { 'is-show': showTypePopover }
        ]"
        class="type-select"
      >
        <template #suffix>
        ...
        if (this.defaultType !== undefined) {
          if (typeof this.defaultType === 'string') {
            ...
            if (this.defaultType.label) {
              ...
              this.customTimes = [
                this.defaultType.startTime,
                this.defaultType.endTime
              ]
  ...
  methods: {
    ...
    sureClick() {
      if (this.showCustomContent) {
        // 自定义时间
        const days = moment(this.editCustomTimes[1].replace(/\./g, '-')).diff(
          moment(this.editCustomTimes[0].replace(/\./g, '-')),
          'days',
          true
        )
        if (days > 90) {
