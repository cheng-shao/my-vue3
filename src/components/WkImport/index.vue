      ...
      <div class="dialog-body">
        ...
        <div v-if="stepsActive == 1" class="step-section">
          ...
          <div v-if="config.repeatHandleShow" class="sections">
            <div class="sections__title">
              二、请选择数据重复时的处理方式<template
                v-if="config.repeatRuleShow"
                >（查重规则<i
                  v-if="helpObj"
                  class="wk wk-icon-fill-help wk-help-tips"
                  :data-type="helpObj.type"
                  :data-id="helpObj.id"
                />{{ `：【${fieldUniqueInfo}】）` }}</template
              >
            </div>
        ...
        >
          <div class="result-info">
            ...
            <p class="result-info__des">
              {{ resultData ? '数据导入完成' : '数据导入失败' }}
            </p>
            <p v-if="resultData" class="result-info__detail">
              ...
              >条，
              <template v-if="repeatHandling === 2"
                >跳过<span class="result-info__detail--suc"
                  >{{ resultData.skipSize }} </span
                >条，</template
              >
              <template v-if="repeatHandling === 1"
                >覆盖<span class="result-info__detail--suc"
                  >{{ resultData.updateSize }} </span
                >条，</template
              >
              导入成功<span class="result-info__detail--suc">{{
                resultData.totalSize - resultData.errSize
              }}</span
              >条<template v-if="repeatHandling === 2">（包含跳过）</template>，
              导入失败<span class="result-info__detail--err">{{
                resultData.errSize
              }}</span
              >条
            </p>
        ...
        <input id="import-input-file" type="file" @change="uploadFile" />
      </div>
      <template #footer>
        <span class="dialog-footer">
          ...
          >
            ...
            />
            <template #reference>
              <el-button class="history-btn" type="text">
                查看历史导入记录
              </el-button>
            </template>
          </el-popover>
...
export default {
    ...
    cacheShow: {
      handler(val) {
        ...
        if (val) {
          ...
          const beforeImportInfo = get('crmImportInfo')
  ...
  methods: {
    ...
    // 选择文件
    selectFile() {
      document.getElementById('import-input-file').click()
    },
    /** 图片选择出发 */
    uploadFile(event) {
