<template>
  <div class="import-history">
    <div class="import-history__hd">
      ...
      <span class="import-history__des"
        >(错误数据只保存七天,七天后将自动失效)</span
      >
    ...
    >
      ...
      <el-table-column prop="address" label="导入结果">
        <template v-slot="scope">
          {{
            `导入总数据${scope.row.title}条，${getImportContent(
              scope.row.title,
              scope.row.content
            )}`
          }}
        ...
        <template v-slot="scope">
          <template v-if="getErrorNum(scope.row.content) > 0">
            <el-button
              ...
              type="primary-text"
              style="padding: 0"
              @click="downloadError(scope.row.messageId)"
              >下载错误数据</el-button
            >
            <span v-else-if="scope.row.valid == 0" class="invalid-tips"
              >已失效</span
            >
          </template>
        </template>
