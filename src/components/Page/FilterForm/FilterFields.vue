<template>
  <div ref="filterFieldsForm" class="filter-fields">
    <el-row v-for="(formItem, index) in form" :key="index" type="flex">
      <el-col :span="6">
        <el-select
      ...
      <template
        v-if="showCalCondition(formItem.formType, formItem.fieldName, formItem)"
      >
      ...
      <el-col
        ...
        :span="1"
      />
      <el-col v-if="formItem.formType == 'business_cause'" :span="4">
        <el-select
      ...
      <el-col
        :span="
          getValueSpan(formItem.formType, formItem.fieldName, formItem)
            ? isExport || !showExport
              ? 11
              : 9
            : isExport
      ...
      >
        ...
        <template v-else-if="updateFlag">
          ...
          <el-select
            v-else-if="
              ['select', 'checkbox', 'field_tag'].includes(formItem.formType)
            "
            v-model="formItem.value"
            :multiple="formItem.formType !== 'field_tag'"
            placeholder="请选择筛选条件"
          ...
          <template
            v-else-if="
              formItem.formType === 'date' || formItem.formType === 'datetime'
            "
          >
            <el-date-picker
      ...
      <el-col v-if="!isExport" :span="1" class="delete">
  ...
  methods: {
    ...
    showCalCondition(formType, fieldName, formItem) {
      return (
        this.getAdvancedFilterOptions(
          this.conditionTypeFun ? this.conditionTypeFun(formItem) : formType,
          fieldName
        ).length > 0
      )
    },
    ...
    selectChange(event, formItem) {
      this.getAdvancedFilterOptions(
        this.conditionTypeFun
          ? this.conditionTypeFun(formItem)
          : formItem.formType,
        formItem.fieldName
      ).forEach((item) => {
        if (item.value === event) {
    ...
    fieldChange(formItem) {
      ...
      if (obj) {
        ...
        this.getAdvancedFilterDefaultItemByFormType(
          formItem,
          obj,
          this.conditionTypeFun
        )
