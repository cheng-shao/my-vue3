  ...
  <div class="wrapper">
    <ul class="list">
      <li v-for="(item, index) in showObj.form" :key="index" class="list-item">
        <span v-if="item.formType == 'date' || item.formType === 'datetime'">{{
          item.name + ' ' + getConditionName(item) + `${getDateContent(item)}`
        }}</span>
        <span v-else-if="item.formType == 'position'">{{
          item.name + ' “' + `${getPositionContent(item)}` + '”'
        }}</span>
        <span v-else-if="item.formType == 'boolean_value'"
          >{{ item.name + ' ' + getConditionName(item) }}“<el-switch
            :value="item.value"
            disabled
            active-value="1"
            inactive-value="0"
          />”</span
        >
        <span v-else-if="item.formType === 'business_cause'">{{
          item.name + ' “' + getTypesName(item) + getStatusName(item) + '”'
        }}</span>
        <span v-else-if="item.formType === 'map_address'">{{
          `${item.name} ${item.address.state} ${item.address.city} ${item.address.area}`
        }}</span>
        <span v-else-if="item.formType === 'checkStatus'">{{
          item.name + ' ' + getConditionName(item) + getNameValue(item)
        }}</span>
        <span v-else-if="item.formType === 'dealStatus'">{{
          item.name + ' ' + getConditionName(item) + getDealStatus(item)
        }}</span>
        <span
          v-else-if="
  ...
  methods: {
    ...
    getValueContent(data) {
      ...
      if (
        ...
        if (data.type === 14) {
          return `“${isEmpty(data.min) ? 0 : data.min}-${
            isEmpty(data.max) ? 0 : data.max
          }”`
        } else {
          return `“${data.value}”`
        }
