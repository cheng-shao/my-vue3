...
export default {
  ...
  watch: {
          ...
          if (this.editId) {
            ...
            for (const field in this.obj.obj) {
              ...
              if (
                ...
                if (item.type === 14) {
                  if (isArray(element.values) && element.values.length > 0) {
                    if (element.values.length === 1) {
                      ...
                      const timeObj = this.timeTypes.find(
                        (tItem) => tItem.value === item.timeType
                      )
                      if (timeObj) {
                ...
                if (item.type === 14) {
                  item.min =
                    isArray(element.values) && !isEmpty(element.values[0])
                      ? element.values[0]
                      : ''
                  item.max =
                    isArray(element.values) && !isEmpty(element.values[1])
                      ? element.values[1]
                      : ''
                } else {
                  item.value = element.values[0]
                ...
                if (element.name === 'settingName') {
                  item =
                    form.find((item) => item.formType === 'business_cause') ||
                    item
    ...
    form() {
      this.$nextTick(() => {
        const container = document.getElementById('scene-filter-container')
  ...
  methods: {
    ...
    handleConfirm() {
      ...
      for (let i = 0; i < this.form.length; i++) {
        ...
        } else if (o.formType == 'date' || o.formType == 'datetime') {
