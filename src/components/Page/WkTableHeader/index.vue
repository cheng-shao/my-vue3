<template>
  <div class="wk-table-header">
    ...
    >
      ...
      <template #right>
        ...
        >
          ...
          <i
            :class="['el-icon-arrow-up', { 'is-reverse': !exportFieldsShow }]"
          />
        </el-button>
        <el-button
          ...
          >
            {{ filterValidCount
              ...
              class="el-icon-close"
              @click.stop="clearFilterVal(filterObj.form)"
            /> </span
        ></el-button>
        <!-- <el-button
    ...
    <!-- 场景 -->
    <scene-set
      v-model:dialog-visible="sceneSetShow"
      :props="sceneSetProps"
    ...
    <filter-form
      v-if="config.showFilterView"
      :field-list="fieldList"
      v-model:dialog-visible="filterShow"
      :obj="filterObj"
      :condition-type-fun="conditionTypeFun"
...
export default {
  ...
  computed: {
    ...
    filterValidCount() {
      ...
      const hasValueConditions = conditions.filter(
        (item) =>
          (item.values && item.values.length > 0) || [5, 6].includes(item.type)
      )
  ...
  watch: {
    ...
    'config.showExportFields': {
      async handler(val) {
        ...
        if (
          this.config.showFilterView &&
          (!isArray(this.fieldList) || this.fieldList.length === 0)
        ) {
          this.initFieldList()
  ...
  methods: {
     ...
     */
    collapseHandle(show) {
      const exportFieldsShow =
        show !== undefined ? show : !this.exportFieldsShow
      if (exportFieldsShow !== this.exportFieldsShow) {
        this.exportFieldsShow = exportFieldsShow
        //  true：是折叠不请求数据 更新table高度
        this.$emit(
          'event-change',
          'export-fields-collapse',
          this.exportFieldsShow
        )
      }
