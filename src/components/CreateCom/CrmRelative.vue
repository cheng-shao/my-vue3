<template>
  ...
  >
    <div ref="crContainer" class="cr-container-wrap">
      <div class="cr-container">
        ...
        >
          <div
            ...
            :key="index"
            :class="
              leftType === item.type ? 'side-item-select' : 'side-item-default'
            "
            class="side-item"
            @click="sideClick(item)"
          >
            {{ item.name }}
            <span v-if="item.num > 0" class="side-item__num">{{
              `(${item.num})`
            }}</span>
          </div>
          <div v-if="leftType == 'dept'">
            <el-tree :data="deptTreeList" @node-click="handleDeptClick" />
          </div>
        </div>
        ...
        <div v-if="commonConfig.isCommon">
          ...
          <crm-relative-table
            ...
            @selection-change="
              selectionChange(
                $event,
                leftSides.find((o) => o.type === leftType)
              )
            "
          />
        </div>
        <div v-else :style="{ 'padding-left': crmType == '' ? '150px' : '0' }">
          <keep-alive
            :include="leftSides.filter((o) => o.loaded).map((o) => o.component)"
          >
            <component
      ...
      <div :class="['dialog-footer', { 'is-common': commonConfig.isCommon }]">
        <el-button type="primary" @click="confirmClick(false)">确定</el-button>
...
export default {
  ...
  emits: ['cancel', 'close', 'update:visible', 'changeCheckout'],
  components: {
  ...
  computed: {
     ...
     */
    contentWidth() {
      return this.crmType == '' && !this.commonConfig.isCommon
        ? '1050px'
        : '910px'
    }
  },
  ...
  methods: {
    ...
    confirmClick() {
      // 如果是单类型 在这里数据赋值
      if (this.crmType) {
        this.currentSelectedData[this.leftType] =
          this.getCurrentMainTable().selectionList
      }
