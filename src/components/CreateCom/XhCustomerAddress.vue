    ...
    <flexbox-item style="margin-right: 50px">
      ...
      >
        <template v-slot="{ item }">
          <div class="name" :title="item.address + item.title">
...
export default {
  ...
  watch: {
    value(newValue) {
      if (
        ...
        !this.valueObjEquals(newValue, this.currentValue)
      ) {
  ...
  mounted() {
    getBaiduMap().then(() => {
      const map = new BMap.Map(this.$refs.addressMap, { enableMapClick: false })
