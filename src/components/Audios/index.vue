  ...
  <div>
    ...
    <div style="display: flex">
      <div class="slider-icon">
        <i v-if="!audio.playing" class="iconfont" @click="startPlayOrPause"
          >&#xe600;</i
        >
      ...
      <div style="width: 100px" class="slider-time">
        ...
        <div v-else-if="!audio.currentTime">
          {{ formatSecond(audio.maxTime) }}
        </div>
        <div v-else>{{ formatSecond(audio.currentTime) }}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    filePath() {
      if (!this.audioUrl) {
        this.config
          .fileRequest(this.config.fileParams)
          .then((res) => {
    ...
    setValue(e) {
      ...
      let value =
        ((e.screenX - $el.getBoundingClientRect().left - 36) / 154) *
        (maxTime - minTime)
      console.log(
        e.screenX,
...
@font-face {
  ...
  src: url('//at.alicdn.com/t/font_1336392_gv2eli7nq6j.eot?#iefix')
