<template>
  <div :class="['xr-radio-menu-wrap', { 'is-show': popverVisible }]">
    <el-popover
    ...
    <!-- 员工部门选择 -->
    <wk-dep-user-dialog
      v-model:user-value="users"
      v-model:dep-value="strucs"
      v-model:visible="membersDepVisible"
      @click.native.stop
...
export default {
  name: 'XrRadioMenu',
  emits: ['update:isDefault', 'update:isDefault', 'input', 'select'],
  components: {
    WkDepUserDialog: defineAsyncComponent(() =>
      import('@/components/NewCom/WkUserDialogSelect/Dialog')
    )
  },
  props: {
    width: {
  ...
  watch: {
    popverVisible(val) {
      if (val) {
        ...
        this.users = objDeepCopy(
          (this.userCheckedData || []).map((item) => item.userId)
        )
        this.strucs = objDeepCopy(
          (this.depCheckedData || []).map((item) => item.deptId)
        )
  ...
  methods: {
    ...
    membersDepSelect(userIds, deptIds, users, strucs) {
      if (!users.length && !strucs.length) {
        ...
        this.selectUsers = users.map((item) => {
          return { userId: item.userId, realname: item.realname }
        })
        this.selectStrucs = strucs.map((item) => {
          return { name: item.name, deptId: item.deptId }
        })
      }
    },
    // 类型选择
