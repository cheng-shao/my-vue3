      ...
      >
        <i
          :class="
            item.examineStatus == 11
              ? 'wk wk-icon-success'
              : getStatusIcon(item.examineStatus)
          "
          :style="{
            color:
              item.examineStatus == 11
                ? '#00875A'
                : getStatusColor(item.examineStatus)
          }"
        ...
        <div>
          ...
          <flexbox class="cf-flow-item-info">
            ...
            <!-- 审核中 3 当做 待审核 0 处理 -->
            <div v-if="item.examineStatus != 11">
              <span>{{ getStatusName(item.examineStatus) }}</span
              >此申请
            </div>
            <div v-else><span>已转审</span></div>
          </flexbox>
