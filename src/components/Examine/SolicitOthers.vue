<template>
  <div class="container">
    ...
    >
      ...
      >
        <el-form-item label="意见审批人" :rules="{ required: true }">
          <flexbox class="user-wrap" wrap="wrap">
            ...
            >
              ...
              <i
                ...
                @click="deleteUserClick(index, solicitOthersForm.userList)"
              />
            </div>
            <div
              v-if="
                solicitOthersForm.userList.length === 0 ||
                solicitOthersForm.userList.length < 20
              "
              class="user"
            >
        ...
        >
          <el-radio
            v-model="solicitOthersForm.examineType"
            class="radio"
            label="1"
          >
            在我之前
          ...
          >
            在我之后
            <span>在我之后审批，即表示我同意该申请并增加审</span>
          </el-radio>
        </el-form-item>
        ...
        >
          <el-radio v-model="solicitOthersForm.moreExamineType" label="1"
            >依次审批</el-radio
          >
          <el-radio v-model="solicitOthersForm.moreExamineType" label="2"
            >会签</el-radio
          >
          <el-radio v-model="solicitOthersForm.moreExamineType" label="3"
            >或签</el-radio
          >
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handlerClick() {
      this.$refs.form.validate((valid) => {
        if (valid && this.solicitOthersForm.userList.length) {
          ...
          if (this.solicitOthersForm.moreExamineType)
            params.type = this.solicitOthersForm.moreExamineType
    ...
    selectUserChange(usersIds, _, users) {
      if (users.length) {
        const userIds = this.solicitOthersForm.userList.map(
          (item) => item.userId
        )
