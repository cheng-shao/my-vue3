...
import { crmExamineRecordAuditAPI, examineEndAPI } from '@/api/examine'
...
export default {
  ...
  emits: ['confirm', 'save', 'emailExamine'],
  components: {},
  ...
  methods: {
    ...
    submitInfo() {
      if (
        (this.status == 2 || this.status == 4 || this.status == 13) &&
        !this.content
      ) {
    ...
    //   }
    //   if (this.status == 1) { // 通过
    //     request = examinePassAPI
    //   } else if (this.status == 2) { // 拒绝
    //     request = examineRejectAPI
    //   } else if (this.status == 13) { // 终止
    //     request = examineEndAPI
    //   }
    //   request(params)
    //     .then(res => {
    ...
    //       this.$emit('save', { type: this.status, examineType: this.examineType })
    //       this.hiddenView()
    //     })
    //     .catch(() => {
    //       this.loading = false
