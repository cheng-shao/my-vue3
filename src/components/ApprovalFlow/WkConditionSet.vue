  ...
  >
    <flexbox v-if="editNode" class="drawer-header">
      <div class="edit-title">
        ...
        />
        <template v-else>
          <span class="title" @click="titleEditClick">{{
            editNode.conditionName
          }}</span>
    ...
    <div v-if="editNode" class="drawer-body">
      ...
      >
        ...
        <el-col :span="17" class="set-row__center">
          <flexbox v-if="item.conditionType === 8">
            <div style="flex: 1">
              <wk-user-dep-dialog-select
                v-model:user-value="item.values.userList"
                v-model:dep-value="item.values.deptList"
          ...
          <template v-else-if="[9, 3].includes(item.type)">
            <el-select
              v-if="item.type == 3"
            ...
            <el-select v-model="item.values" multiple class="condition-value">
              <el-option
        ...
        <el-col :span="1" class="set-row__footer"
          ><i class="wk wk-delete" @click="deleteItem(index)"
        /></el-col>
      </el-row>
      <div>
        <el-button type="text" icon="el-icon-plus" @click="conditionSelectClick"
          >添加条件</el-button
        >
      ...
      this.editNode.conditionDataList = conditionDataList.filter((item) => {
        if (item.type === 3 || item.type === 9) {
          ...
          if (item.conditionType === 6) {
            return (
              !isEmpty(item.leftValue) &&
              !isEmpty(item.rightValue) &&
              item.leftValue < item.rightValue
            )
          } else if (item.conditionType === 8) {
            // 人员部门角色
            return (
              !isEmpty(item.userList) ||
              !isEmpty(item.deptList) ||
              !isEmpty(item.roleList)
            )
          } else {
