    ...
    <div class="condition-node-wrap">
      ...
      >
        <div class="header">
          ...
          <i class="el-icon-close close" @click.stop="deleteClick" />
        </div>
        <div class="content">
          <template
            v-if="node.conditionDataList && node.conditionDataList.length > 0"
          >
            {{ getConditonContent() }}
          </template>
      ...
      return conditionDataList
        .map((item) => {
          if (item.type === 3) {
            ...
            if (item.setting) {
              ...
              item.setting.forEach((setting) => {
                if (
                  ...
                  item.values.includes(setting.value)
                ) {
            ...
            return `${item.name} ${
              this.selectOptionsObj[item.conditionType]
            } ${values.join('或')}`
          } else if (item.type === 9) {
            return `${item.name} ${
              this.checkboxOptionsObj[item.conditionType]
            } ${item.values.join('、')}`
          } else {
            if (item.conditionType === 6) {
              return `${isEmpty(item.leftValue) ? '' : item.leftValue} ${
                this.numberValueOptionsObj[item.leftCondition]
              } ${item.name} ${
                this.numberValueOptionsObj[item.rightCondition]
              } ${isEmpty(item.rightValue) ? '' : item.rightValue}`
            } else if (item.conditionType === 8) {
              return `${item.name}属于 ${item.deptList
                .map((item) => item.name)
                .join('或')}${
                item.deptList.length > 0 && item.userList.length > 0 ? '或' : ''
              }${item.userList.map((item) => item.realname).join('或')}${
                item.userList.length > 0 && item.roleList.length > 0 ? '或' : ''
              }${item.roleList.map((item) => item.roleName).join('或')}`
            } else {
              return `${item.name} ${
                this.numberOptionsObj[item.conditionType]
              } ${item.values}`
            }
          }
        })
