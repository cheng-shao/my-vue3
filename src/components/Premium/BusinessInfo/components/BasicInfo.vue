    ...
    <div v-for="(item, index) in tabs" :key="index">
      ...
      >
        <table v-if="activeTab === 'base'" class="base-table">
          <tbody>
            <tr v-for="(bItem, bIndex) in baseList" :key="bIndex">
              <td>{{ bItem.label }}</td>
              <td class="w-292">{{ getBaseInfoValue(bItem.field) }}</td>
              <td>{{ bItem.label1 }}</td>
              <td class="w-292">
                {{ bItem.field1 ? getBaseInfoValue(bItem.field1) : '' }}
              </td>
            </tr>
            <tr>
              <td>注册地址</td>
              <td colspan="3">{{ getBaseInfoValue('address') }}</td>
            </tr>
            <tr>
              <td>经营范围</td>
              <td colspan="3">{{ getBaseInfoValue('scope') }}</td>
            </tr>
          </tbody>
        </table>
        <el-table
...
export default {
  ...
  methods: {
    ...
    getBaseInfoValue(field) {
      ...
      if (field === 'term') {
        const list = []
        const termStart = this.data.termStart
          ? this.data.termStart.split(' ')[0]
          : ''
        if (termStart) {
          list.push(termStart)
        }
