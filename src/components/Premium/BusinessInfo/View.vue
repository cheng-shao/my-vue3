  ...
  >
    ...
    >
      <flexbox class="business-info__header">
        ...
        <div class="handle">
          <template v-if="config.fillBtnShow">
            <el-button type="primary" @click="fillClick">回填</el-button
            ><i
          ...
          <el-button @click="updateClick">更新</el-button
          ><i
            class="wk wk-icon-fill-help wk-help-tips"
            data-type="8"
            data-id="343"
      ...
      <div v-if="contactDetail" class="abstract">
        ...
        >
          ...
          <span class="value">{{
            contactDetail[item.field] || detail[item.field]
          }}</span>
        </div>
      </div>
      ...
      <el-tabs v-model="activeTab" lazy>
        <el-tab-pane name="overview">
          ...
          <basic-info :key="updateKey" :name="name" :data="detail" />
        </el-tab-pane>
      </el-tabs>
    </div>
    <el-empty
      v-if="errorShow && canUse"
      style="padding-top: 50%; transform: translateY(-50%)"
...
import Pay from '@/views/system/pay'
...
export default {
  ...
  computed: {
    ...
    // 如果有次数就可以使用
    canUse() {
      const enterprise = this.moduleData.find(
        (item) => item.module === 'enterprise' && item.status === 1
      )
  ...
  methods: {
    ...
    getDetail() {
      this.loading = true
      crmEnterpriseBasicInfoAPI(this.name)
        .then((res) => {
    ...
    getContactInfo() {
      this.loading = true
      crmEnterpriseContactInfoAPI(this.name)
        .then((res) => {
    ...
    async fillClick() {
      ...
      if (this.module === 'leads') {
        ...
        this.$set(this.form, 'address', [{ id: 4, name: address, code: '' }])
      } else {
        this.$set(
          this.form,
          'customerName',
          this.getFillValue(this.detail.name)
        )
      }
      this.$set(
        this.form,
        'mobile',
        this.getFillValue(this.contactDetail.telephone)
      )
      this.$set(
        this.form,
        'telephone',
      ...
      this.fields.forEach((item) => {
        if (supportFieldNames.includes(item.name)) {
          const supportField = supportFields.find(
            (sItem) => sItem.name === item.name
          )
          if (supportField) {
            if (
              isArray(supportField.formType) &&
              supportField.formType.includes(item.formType)
            ) {
              const list = []
              const termStart = this.detail.termStart
                ? this.detail.termStart.split(' ')[0]
                : ''
              list.push(termStart || '')
              const termEnd = this.detail.termEnd
                ? this.detail.termEnd.split(' ')[0]
                : ''
              list.push(termEnd || '')
              ...
              this.$set(
                this.form,
                item.fieldName,
                this.getFillValue(this.detail[supportField.field])
              )
            }
          }
        }
    ...
    updateClick() {
      this.loading = true
      crmEnterpriseCleanCacheAPI(this.name)
        .then((res) => {
          this.afterEnter()
