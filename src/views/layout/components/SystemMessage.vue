<template>
  <div v-if="!lazy || loaded || active" v-show="active" class="sm-main">
    ...
    <j-x-c-full-screen-detail
      v-if="showJxcDetail && jxcId && jxcType && !showFullDetail"
      :id="jxcId"
      :type="jxcType"
      @close="showJxcDetail = false"
    />
  </div>
</template>

<script>
import TodayListDetail from '@/views/oa/calendar/components/TodayListDetail'
...
export default {
  ...
  computed: {
    ...mapGetters(['oa', 'crm', 'project', 'knowledge', 'hrm', 'jxc']),
    active() {
    ...
    title() {
      if (this.onlyAnnouncement) {
        return this.unreadNums.announceCount &&
          this.unreadNums.announceCount > 0
          ? `公告（${this.unreadNums.announceCount}）`
          : '公告'
      }
      return this.unreadNums.allCount > 0
        ? `通知（${this.unreadNums.allCount}）`
        : '通知'
    },
  ...
  methods: {
    ...
    checkCRMDetail(type, id, dataIndex, data) {
      if (type === 'schedule') {
        ...
        kmDocDetailRemindAPI({ documentId: id })
          .then((res) => {
            const data = res.data || {}
            if (data.type == 'share') {
              window.open(
                `${WKConfig.getLocationOrigin()}/share/#/${data.data}`
              )
          ...
          .catch(() => {})
      } else if (type.startsWith('jxc_')) {
        this.jxcId = id
        this.jxcType = type
        this.showJxcDetail = true
      } else if (type === 'hrm') {
        if (data.type === 83) {
    ...
    allDeleteClick() {
      if (this.currentMenu) {
        const name =
          this.currentMenu.label == 'all' ? '' : this.currentMenu.name
