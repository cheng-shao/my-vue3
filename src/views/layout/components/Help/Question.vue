<template>
  <div class="main">
    <div class="main-title">系统功能介绍</div>
    <div class="main-des">
      为了帮助您更好地使用悟空CRM，现对系统功能做进一步介绍，其中包括操作方法、术语解释、数据来源等。
    </div>
    <div class="main-filter">
      <el-select v-model="moduleId" mode="no-border" @change="moduleChange">
        <el-option
      ...
      <el-select v-model="menuId" mode="no-border" @change="getData">
        <el-option
...
export default {
  ...
  methods: {
     ...
     */
    moduleChange() {
      this.options = this.moduleList.find(
              ...
              if (this.qId) {
                ...
                setTimeout(() => {
                  const findIndex = this.list.findIndex(
                    (item) => item.id == this.currentMenuId
                  )
                  if (findIndex >= 5) {
